...
public class SuccessHistoryVoiceJob implements Job {
    ...
    //本地的目录前后必须有 "/"
    private static String successFtpVoicePath = "/home/mhn/voiceftp/success/";
    ...
    private Logger logger = Logger.getLogger(SuccessHistoryVoiceJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {

        System.out.println("******************判断是否开始历史成功单任务************************");
        logger.info("******************判断是否开始历史成功单任务************************");


        String dayNum = repository.findOne(8L).getCron();// 从数据库查询出来的
        String time = repository.findOne(9L).getCron();// 从数据库查询出来的
        String flag = repository.findOne(10L).getCron();// 从数据库查询出来的
        if (flag.equals("on")) {
            System.out.println("******************++++开始历史成功单任务++++************************");
            logger.info("******************++++开始历史成功单任务++++************************");
            ...
            for (int x = 0; x < Integer.parseInt(dayNum); x++) {
                ...
                //执行过程中的关闭
                String flagExce = repository.findOne(10L).getCron();// 从数据库查询出来的
                if (flagExce.equals("off")) {
                    System.out.println("读取" + nowTime.getTime() + "的历史成功单数据已经关闭");
                    logger.info("读取" + nowTime.getTime() + "的历史成功单数据已经关闭");
                    return;
                }
                //本地文件名
                String localName = "";
                ...
                sftpClient.setDirectory("/success");

                String fileName = DateConversion.DateToStr(nowTime.getTime(),"yyyyMMdd") + "_success_";
                String fileNameSend = "0";


                SuccessModal model = new SuccessModal();
                ...
                try {
                    //计算一共导出多少个fsr包
                    Grid suGrid = voiceService.getSuccessList(model, new PageFilter(0,10));
                    long total =  suGrid.getiTotalDisplayRecords();
                    int sheetNum =(int)total/totalNum+1;
                    if (total == 0) {
                        continue;
                    }
                    System.out.println("一共导出zip包为：" + sheetNum +"**************success历史**************，一共的个数为" + total + "**************" + nowTime.getTime());
                    logger.info("一共导出zip包为：" + sheetNum +"**************success历史**************，一共的个数为" + total + "**************" + nowTime.getTime());


                    for (int i = 1;i <= sheetNum; i++){
                        System.out.println("导出zip包：" + fileName + i + "_" + fileNameSend +"**************success历史**************"+ nowTime.getTime());
                        logger.info("导出zip包：" + fileName + i + "_" + fileNameSend +"**************success历史**************"+ nowTime.getTime());
                        ...
                        for(int z=0 ; z < j; z++){
                            //拼凑数据
                            Grid grid = voiceService.getSuccessList(model, new PageFilter((i-1)*totalNum + z*singleNum , singleNum));
                            //处理失败第二次调用
                            if ("false".equals(grid.getMessage())) {
                                grid = voiceService.getSuccessList(model, new PageFilter((i-1)*totalNum + z*singleNum , singleNum));
                                System.out.println("【成功单历史超时，第二次调用】");
                                logger.info("【成功单历史超时，第二次调用】");
                            }
                        ...
                        System.out.println("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************success历史**************"+ nowTime.getTime());
                        logger.info("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************success历史**************"+ nowTime.getTime());
                        ...
                        System.out.println("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************success历史**************"+ nowTime.getTime() + "***************耗时为：" + DateConversion.formatTime(time5 - time4));
                        logger.info("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************success历史**************"+ nowTime.getTime() + "***************耗时为：" + DateConversion.formatTime(time5 - time4));

                        System.out.println("发送zip包：" + fileName + i + "_" + fileNameSend +"**************success历史**************"+ nowTime.getTime());
                        logger.info("发送zip包：" + fileName + i + "_" + fileNameSend +"**************success历史**************"+ nowTime.getTime());

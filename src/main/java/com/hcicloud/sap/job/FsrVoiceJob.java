...
public class FsrVoiceJob implements Job {
    ...
    private Logger logger = Logger.getLogger(FsrVoiceJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        System.out.println("******************防骚扰任务开始************************");
        logger.info("******************防骚扰任务开始************************");
        ...
        String fileName = DateConversion.DateToStr(DateConversion.getDayBeforeBegin(nowTime.getTime(),1),"yyyyMMdd") + "_fsr_";
        String fileNameSend = "0";

        //TODO 10月1号，日期为0的问题需要修复
...
//        model.setStartTime("2017-09-30 00:00:00");
//        model.setEndTime("2017-09-30 23:59:59");

        try {
            //计算一共导出多少个fsr包
            Grid suGrid = voiceService.getFsrList(model, new PageFilter(0,10));
            long total =  suGrid.getiTotalDisplayRecords();
            int sheetNum =(int)total/totalNum+1;
            if (total == 0) {
                return;
            }
            System.out.println("一共导出zip包为：" + sheetNum +"**************fsr**************，一共的个数为" + total + "**************" + nowTime.getTime());
            logger.info("一共导出zip包为：" + sheetNum +"**************fsr**************，一共的个数为" + total + "**************" + nowTime.getTime());
            ...
            for (int i = 1;i <= sheetNum; i++){
                ...
                for(int z=0 ; z < j; z++){
                    //拼凑数据
                    Grid grid = voiceService.getFsrList(model, new PageFilter((i-1)*totalNum + z*singleNum , singleNum));
                    //处理失败第二次调用
                    if ("false".equals(grid.getMessage())) {
                        grid = voiceService.getFsrList(model, new PageFilter((i-1)*totalNum + z*singleNum , singleNum));
                        System.out.println("【防骚扰超时，第二次调用】");
                        logger.info("【防骚扰超时，第二次调用】");
                    }
...
//                    Thread.sleep(500);
                ...
                System.out.println("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************fsr**************"+ nowTime.getTime());
                logger.info("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************fsr**************"+ nowTime.getTime());
                ...
                System.out.println("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************fsr**************"+ nowTime.getTime() + "***************耗时为：" + DateConversion.formatTime(time5 - time4));
                logger.info("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************fsr**************"+ nowTime.getTime() + "***************耗时为：" + DateConversion.formatTime(time5 - time4));

                System.out.println("发送zip包：" + fileName + i + "_" + fileNameSend +"**************fsr**************"+ nowTime.getTime());
                logger.info("发送zip包：" + fileName + i + "_" + fileNameSend +"**************fsr**************"+ nowTime.getTime());

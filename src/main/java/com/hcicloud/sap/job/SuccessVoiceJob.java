...
public class SuccessVoiceJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        String fileName = DateConversion.DateToStr(DateConversion.getDayBeforeBegin(nowTime.getTime(),1),"yyyyMMdd") + "_success_";
...
//        model.setStartTime("2017-09-23 00:00:00");
//        model.setEndTime("2017-09-23 23:59:59");
        ...
        try {
            ...
            for (int i = 1;i <= sheetNum; i++){
                System.out.println("导出zip包：" + fileName + i + "_" + fileNameSend +"**************success**************"+ nowTime.getTime());
                logger.info("导出zip包：" + fileName + i + "_" + fileNameSend +"**************success**************"+ nowTime.getTime());
                ...
                for(int z=0 ; z < j; z++){
                    ...
                    if ("false".equals(grid.getMessage())) {
                        ...
                        System.out.println("【成功单超时，第二次调用】");
                        logger.info("【成功单超时，第二次调用】");
                    }
                    ...
                    for (VoiceVo voiceVo : aaData) {
                        ...
                        String userPhoneTemp = voiceVo.getUserPhone();
                        String callContentTemp = voiceVo.getCallContent();
                        String callContent = "";

                        //话者分离的格式
                        if (StringUtils.isNotEmpty(userPhoneTemp) || StringUtils.isNotEmpty(callContentTemp)) {
                            callContent = CommonMethod.transContent(userPhoneTemp,callContentTemp);
                        }
                ...
                System.out.println("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************success**************"+ nowTime.getTime());
                logger.info("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************success**************"+ nowTime.getTime());
                ...
                System.out.println("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************success**************"+ nowTime.getTime() + "***************耗时为：" + DateConversion.formatTime(time5 - time4));
                logger.info("压缩zip包：" + fileName + i + "_" + fileNameSend +"**************success**************"+ nowTime.getTime() + "***************耗时为：" + DateConversion.formatTime(time5 - time4));

                System.out.println("发送zip包：" + fileName + i + "_" + fileNameSend +"**************success**************"+ nowTime.getTime());
                logger.info("发送zip包：" + fileName + i + "_" + fileNameSend +"**************success**************"+ nowTime.getTime());
